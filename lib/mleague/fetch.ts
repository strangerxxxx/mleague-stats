import type { CachedGames, Game, SeasonRef } from "./types";
import {
  currentSeasonMonthUrls,
  currentSeasonRefreshUrls,
  discoverSeasons,
  mergeGames,
  parseGamesHtml,
} from "./parse";
...
export { loadCachedGames, saveCachedGames } from "./store";
...
export async function fetchAllGames(options?: {
  ...
  for (const season of seasons) {
    ...
    const cachedCurrent = cachedBySeason.get(season.id) ?? [];
    const incremental = cachedCurrent.length > 0 && !options?.refreshAll;
    const monthUrls = incremental
      ? currentSeasonRefreshUrls()
      : currentSeasonMonthUrls(indexHtml, season.id);
    console.info(
      `Fetching ${season.id} (${incremental ? "current months only" : "all months"}): ${monthUrls.join(", ")}`,
    );
    ...
    htmlGroups.push(incremental ? mergeGames([cachedCurrent, ...monthGames]) : mergeGames(monthGames));
  }
...
function fetchCache(revalidateSeconds: number): RequestInit {
